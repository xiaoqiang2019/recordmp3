...
<!-- 在需要录音功能的页面引入`app.js`文件（src内的为源码、dist内的为压缩后的）即可。
...
import RecordApp from 'recorder-core/src/app'
//可选开启Native支持，需要引入此文件
import 'recorder-core/src/app-native-support'
//可选开启IOS上微信录音支持，需要引入此文件
import 'recorder-core/src/app-ios-weixin-support'
...
使用本功能虽然可以最大限度的兼容`Android`和`IOS`，但使用[app-ios-weixin-support.js](../src/app-ios-weixin-support.js)需要后端提供支持，如果使用[app-native-support.js](../src/app-native-support.js)需要App端提供支持，具体情况查看这两个文件内的注释。
...
## 统一实现参考
每个底层平台都实现了三个方法，`Native`在[app-native-support.js](https://github.com/xiangyuecn/Recorder/blob/master/src/app-native-support.js)中实现了，`IOS-Weixin`在[app-ios-weixin-support.js](https://github.com/xiangyuecn/Recorder/blob/master/src/app-ios-weixin-support.js)中实现了，`Default`在[app.js](https://github.com/xiangyuecn/Recorder/blob/master/src/app.js)中实现了。
...
使用App原生录音，必需提供配置中的`IsApp`、`JsBridgeRequestPermission`、`JsBridgeStart`、`JsBridgeStop`方法，具体情况请查阅[src/app.js](https://github.com/xiangyuecn/Recorder/blob/master/src/app.js)内有详细的说明。
...
使用微信录音，必需提供配置中的`WxReady`、`DownWxMedia`方法，可选提供`Enable`方法，具体情况请查阅[src/app.js](https://github.com/xiangyuecn/Recorder/blob/master/src/app.js)内有详细的说明。
